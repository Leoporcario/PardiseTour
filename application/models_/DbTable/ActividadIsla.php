...
class Model_DBTable_ActividadIsla extends Zend_Db_Table_Abstract {
    
    private static $instance;
    protected $_name = 'adm_actividad-isla';
    protected $primary = 'idActividadIsla';
    protected $defultSort = 'ISnombreEs';
    protected $defultOrder = 'ASC';

    public static function getInstance() {
        if (!isset(self::$instance)) {
            self::$instance = new Model_DBTable_ActividadIsla();
        }
    ...
    public function get($id) {
        ...
        $select->setIntegrityCheck(false);
        $select->from(array($this->_name), array("*"));
        $select->where('idActividadIsla = ' . $id);
