...
class Admin_ArchivoController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $this->archivo_perfil = new Model_DBTable_ArchivoPerfil();
    ...
    public function addAction(){
        ...
        if ($this->getRequest()->isPost()) {
            if (empty($_POST['AnombreEs']) && empty($_POST['AnombreEn'])) {
                $this->_helper->flashMessenger->addMessage(array('type' => 'danger', 'message' => "Debe ingresar al menos el nombre en un idioma"));
                $this->_redirector->gotoSimple('add', 'archivo', null, array('status' => 'warning'));
            }
            if ($_FILES["AfisicoEs"]["error"]!=0 && $_FILES["AfisicoEn"]["error"]!=0) {
                $this->_helper->flashMessenger->addMessage(array('type' => 'danger', 'message' => "Debe seleccionaral menos un archivo"));
                $this->_redirector->gotoSimple('add', 'archivo', null, array('status' => 'warning'));
            }
            ...
            try{
                ...
                unset($_POST['perfiles']);
                $IDArchivo=$this->archivos->add($_POST);
                if($perfiles){
                    foreach ($perfiles as $key => $IDPerfil) {
                        $this->archivo_perfil->add(array('IDArchivo'=>$IDArchivo,'IDPerfil'=>$IDPerfil));
                    }
                }   
                ...
                $this->_redirector->gotoSimple('add', null, null, array('status' => 'danger'));
            }
        }        
    ...
    public function editAction(){   
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (empty($_POST['AnombreEs']) && empty($_POST['AnombreEn'])) {
                $this->_helper->flashMessenger->addMessage(array('type' => 'danger', 'message' => "Debe ingresar al menos el nombre en un idioma"));
                $this->_redirector->gotoSimple('edit', 'archivo', null, array('id' => $this->view->parameters['id']));
            }
            unset($_POST['submit']);
            try{ 
                ...
                //Guardo la fecha de actualizacion
                $_POST['AfechaActualizacion'] = date('Y-m-d H:i:s');
                $_POST['idArchivo'] = $archivo['idArchivo'];
                $perfiles=$_POST["perfiles"];
                unset($_POST['perfiles']);

                $this->archivos->edit($_POST);
                $this->archivo_perfil->delete_row_by_archivo($_POST['idArchivo']);
                if($perfiles){
                    foreach ($perfiles as $key => $IDPerfil) {
                        $this->archivo_perfil->add(array('IDArchivo'=>$_POST['idArchivo'],'IDPerfil'=>$IDPerfil));
                    }
                }   
                //Manejo avisos y redirecciono
                $this->_helper->flashMessenger->addMessage(array('type' => 'success', 'message' => $this->newSuccessMessage));
        ...
        $this->view->perfiles = $this->perfiles->showAll();
        $this->view->perfiles_seleccionados = $this->archivo_perfil->getByArchivo($this->view->parameters['id']);
            ...
            if ($this->form->isValid($this->getRequest()->getPost())) {
                ...
                if ($delete_request == "Yes") {
                    try {
                        ...
                        Model_DBTable_ArchivoPerfil::getInstance()->delete_row_by_archivo($id);
