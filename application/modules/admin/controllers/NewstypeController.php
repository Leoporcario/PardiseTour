...
class Admin_NewstypeController extends Zend_Controller_Action {
    
    var $fields = array(
        array('field' => 'IDTipoNoticia', 'label' => 'ID ', 'list' => true, 'search' => true, 'width' => 60, 'class' => 'id', 'order' => true),
        array('field' => 'TNNombre', 'label' => 'Tipo noticia', 'list' => true,'search' => true)
    );
    ...
    public function init() {
        ...
        $this->view->currentBrand = 'Tipo de Noticias';
    ...
    public function indexAction(){
        ...
        try {
            $results_all = $this->newsTypes->showAll($where, $this->parameters['sort'], $this->parameters['order']);
    ...
    public function addAction(){
        ...
        $this->view->icon = 'plus';
        $this->view->title = 'Agregar nuevo tipo de noticia';
        $this->view->description = 'Complete el formulario para agregar un nuevo tipo';
    ...
    public function editAction(){   
        $noticia = $this->newsTypes->get($this->view->parameters['id']);
        if ($this->getRequest()->isPost()) {
            //Deleting variables from post
            unset($_POST['submit']);
            try{ 
        ...
        $this->view->title = 'Editar tipo de noticia';
        $this->view->description = 'Complete el formulario para editar este tipo';
   ...
   public function deleteAction() {
        ...
        if ($this->getRequest()->isPost()) {
            if ($this->form->isValid($this->getRequest()->getPost())) {
                ...
                if ($delete_request == "Yes") {
                    try {
                        $this->view->result = $this->newsTypes->get($id);
                        ...
                        $this->view->result = $this->newsTypes->delete_row($id);
                    } catch (Zend_Exception $exc) {
        ...
        if ($id > 0) {
            ...
            $this->_redirect('admin/newstype/');
        }
        
        //Icono, titulo y descripcion para el formulario
        $this->view->icon = 'ban-circle';
        $this->view->title = 'Eliminar tipo de noticia';
